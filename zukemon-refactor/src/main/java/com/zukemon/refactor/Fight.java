...
public class Fight {
    ...
    public Zukemon fight(FightType fightType) {
        ...
        if (fightType == FightType.NORMAL) {
            return fightNormal();
        } else if (fightType == FightType.DEFEND) {
            return fightDefend();
        } else if (fightType == FightType.ROYAL_RUMBLE) {
            return fightRoyalRumble();
        } else {
            throw new IllegalArgumentException("FightMode '" + fightType + "' is not a valid or know fight mode");
        }
    }

    private Zukemon fightRoyalRumble() {
